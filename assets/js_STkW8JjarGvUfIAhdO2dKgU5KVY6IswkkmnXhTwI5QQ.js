...
(function ($, Drupal) {
    ...
    Drupal.behaviors.newViewFocusOnAjax = {
      ...
      // Suppress error message of file upload progress when file validation fails.
      $('.webform-submission-contact-us-form').on('submit', function (event) {
        var $form = $(this);
  ...
  function selectUnselectall(view_id, all_option, all_selector) {
    var flag = true;
    $(view_id + ' ' + all_option).find('div:not(' + all_selector + ')').find('input').each(function () {
      if (!$(this).prop('checked')) {
        flag = false;
      }
    });
    if (flag == true) {
      $(all_option).find('div'+ all_selector).find('input').prop('checked', 'checked');
    }
    else {
      $(all_option).find('div'+ all_selector).find('input').removeAttr('checked');
    }
  }
  ...
  Drupal.behaviors.nestle = {
    attach: function (context, drupalSettings, settings) {
      ...
      if(!($('#block-exposedformglobal-searchglobal-search-block-all')).length) {
      ...
      $('.view-id-map_locator .ui-accordion-header').once().on('click', function() {
        var _countryCode = $(this).find('.map-locator-county').attr("data-country");
        var _this = $(this);
        var positon = $(document).scrollTop();
        $('#views-exposed-form-map-locator-office-locations select').val(_countryCode).once().trigger('change');
        _countryCode = $('#views-exposed-form-map-locator-office-locations select').val();
        // Initialy hide all accordion.
        hideAccordion();
        $(_this).next().css('display', 'block');
        $(_this).addClass('ui-accordion-header-active ui-state-active');
        $(document).ajaxComplete(function(event, xhr, settings) {
          // After ajax complete hide the active accordion.
          hideAccordion();
          // Find active accordion after ajax complete.
          $( ".view-id-map_locator .view-content .views-row" ).each(function( index ) {
            // Match active accordion with active map and open.
            if($(this).find('.ui-accordion-header .map-locator-county').attr('data-country') == _countryCode ) {
              var activeAccordion = $(this).find('.ui-accordion-header .map-locator-county').closest('.ui-accordion-header');
              $(activeAccordion).addClass('ui-accordion-header-active ui-state-active');
              $(activeAccordion).next().css('display', 'block');
            }
          });
          if(isIE) {
            $('html,body').animate({ scrollTop: positon }, 'fast');
            return false
          }
        });
      });
      // Filters made more understandable based on apllication.
      if ($('.view-map-locator div[id*="edit-char"]') != null) {
        var _allCountry = $('.view-map-locator div[id*="edit-char"]');
        $(_allCountry).find('.form-item').once().each(function() {
          var _country = $(this).find('input');
          if ($(_country).val() != 'All') {
            var _countryName = Drupal.t('Country name starting with ') + $(_country).val();
            $(_country).attr('aria-label', _countryName);
          }
        });
      }
      ...
      $('.form-item-zone ').once().on('click', function() {
        $(document).ajaxComplete(function(event, xhr, settings) {
          $('#views-exposed-form-map-locator-office-locations select').val('All').click();
          hideAccordion();
        });
      });
      try {
      ...
      var view_id = '#views-exposed-form-article-list-search-all-news';
      var all_option = 'div[data-drupal-selector="edit-type"]';
      var all_selector = '.form-item-type-all';

      selectUnselectall(view_id, all_option, all_selector);

      $(view_id + ' ' + all_option).find('div:not(' + all_selector + ')').find('input').on('click', function() {
        selectUnselectall(view_id, all_option, all_selector);
      });
      ...
      $('.view-id-ln_c_mosaic .pager .pager__item .counter').once().before("<span class='arrow'></span>");
      $('#views-exposed-form-ln-c-mosaic-mosaic-block .form-type-radio input:checked')
          .closest('.form-type-radio').addClass('is-checked');
      // Browse by Tags toggle
      $('#views-exposed-form-ln-c-mosaic-mosaic-block .browse-tags', context).once().on('click', function() {
        $('div[id*="edit-field-story-category-target-id"] .form-type-radio input[type="radio"]').removeAttr('checked');
        $('div[id*="edit-field-story-category-target-id"] .form-type-radio').removeClass('is-checked');
        $('div[id*="edit-field-topic-target-id"] .form-type-radio input[type="radio"]').removeAttr('checked');
        $('div[id*="edit-field-story-category-target-id"] .form-type-radio:first-child input[type="radio"]').trigger('click');
        $('div[id*="edit-field-topic-target-id"] .form-type-radio:first-child input[type="radio"]').trigger('click');
        $('#views-exposed-form-ln-c-mosaic-mosaic-block .form-type-radio').removeAttr('checked');
        $(document).ajaxComplete(function(event, xhr, settings) {
          $('#views-exposed-form-ln-c-mosaic-mosaic-block .fieldset-wrapper').show();
          $('div[id*="edit-field-story-category-target-id"] .form-type-radio .is-checked').removeClass('is-checked');
        });
      });
      $('#views-exposed-form-map-locator-office-locations .form-type-radio input:checked')
        .closest('label').addClass('is-checked');
      $('div[id*="edit-field-story-category-target-id"] .form-type-radio').once().on('click', function() {
        $('div[id*="edit-field-topic-target-id"] .form-type-radio input[type="radio"]').removeAttr('checked');
        $(document).ajaxComplete(function(event, xhr, settings) {
          $('#views-exposed-form-ln-c-mosaic-mosaic-block .fieldset-wrapper').hide();
          $('#views-exposed-form-ln-c-mosaic-mosaic-block .form-type-radio input:checked')
          .closest('.form-type-radio').addClass('is-checked');
        });
      });
      $('div[id*="edit-field-topic-target-id"] .form-type-radio').once().on('click', function() {
        $(document).ajaxComplete(function(event, xhr, settings) {
          $('#views-exposed-form-ln-c-mosaic-mosaic-block .fieldset-wrapper').show();
          $('div[id*="edit-field-story-category-target-id"] .form-type-radio').removeClass('is-checked');
        });
      });
      $('#views-exposed-form-map-locator-case-studies .select-wrapper, .view-display-id-office_location_attachment .select-wrapper, ' +
        '#views-exposed-form-documents-reports-block-documents-reports .select-wrapper, #views-exposed-form-ln-c-mosaic-mosaic-block .select-wrapper')
        .removeClass('select-wrapper');
      $('.webform-submission-contact-us-form .select-wrapper')
          .removeClass('select-wrapper').addClass('contact-select-wrapper');
      ...
      $('#views-exposed-form-ln-c-mosaic-mosaic-block .form-type-radio input:checked')
       .closest('.form-type-radio').addClass('is-checked');
      $('div[id*="edit-field-story-category-target-id"] input[type="radio"], div[id*="edit-field-topic-target-id"] input[type="radio"]').once().click(function() {
        var positon = $(document).scrollTop();
        $('button[id*="edit-submit-ln-c-mosaic"]').trigger('click');
          if(isIE) {
            $(document).ajaxComplete(function(event, xhr, settings) {
              $('html,body').animate({ scrollTop: positon }, 'fast');
              return false;
      ...
      $('select[id*="edit-field-story-category-target-id"]').once().change(function() {
        $('select[id*="edit-field-topic-target-id"]').prop("selectedIndex", 0);
        $('button[id*="edit-submit-ln-c-mosaic"]').trigger('click');
      });
      $('select[id*="edit-field-topic-target-id"]').once().change(function() {
        $('select[id*="edit-field-story-category-target-id"]').prop("selectedIndex", 0);
        $('button[id*="edit-submit-ln-c-mosaic"]').trigger('click');
      });
      $(".view-display-id-block_events .add-to-cal").click(function(e) {
        e.stopPropagation();
      });
      $(".view-display-id-block_events tbody tr").click(function() {
      ...
      $(document).once().ready(function() {
        ...
        if( _searchedValue ) {
          ...
          if(checkUrl('year')) {
            $('.view-display-id-block_press_releases .slick-slider .year-selected .form-radio').focus();
          }
        }
        $('.js-views-accodion-group-header').attr('tabindex', '0');
      });
      ...
      $(document).ajaxComplete(function(event, xhr, settings) {
        ...
        // After ajax complete load the map and content.
        $('.view-id-map_locator .views-row').css('display', 'block');
      ...
      if($('[id^="edit-year"] .form-type-radio .form-radio:checked')) {
        $('[id^="edit-year"] .form-type-radio .form-radio:checked', context)
            .closest('.control-label').addClass('year-selected');
      }
      ...
      $(".social-links-wrapper .social-platforms > a").once().on('click', function(e) {
        ...
        if ($(window).width() < 1024) {
          e.preventDefault();
          if ($(this).next('.social-links').hasClass('active-icon')) {
            $(".social-links-wrapper .social-platforms > .social-links").removeClass('active-icon');
          }
          else {
            $(".social-links-wrapper .social-platforms > .social-links").removeClass('active-icon');
            $(this).next('.social-links').addClass('active-icon');
          }
        }
      });
      ...
      $('.webform-submission-contact-us-form #edit-country-code-field').focusout(function() {
        var _val = $(this).val();
        if (_val) {
          if (_val.indexOf("+") === 0) {
            if (_val.charAt(1) == " ") {
              _val = _val.substr(0, 1) + _val.substr(2);
            }
          }
          else {
            _val = "+" + _val;
      ...
      // IE sepecific css on file upload contact-us form
      $("[id^=edit-attach-a-file-2mb-limit-upload]").change(function() {
         if($("[id^='edit-attach-a-file-2mb-limit-']").find('.messages--error')) {
          $('.form-type-managed-file #edit-attach-a-file-2mb-limit---label').addClass('error-message-btn');
         }
      });
      ...
      if($('.paragraph').hasClass('paragraph--type--dsu-c-view')) {
        $('.field--name-field-components .paragraph--type--dsu-c-view .component-paragraph-field, .field--name-field-terms-components .paragraph--type--dsu-c-view .component-paragraph-field').once().each(function (index, value) {
          var _this = $(this);
          var view_class = $(_this).find('.views-element-container .view').attr('class');
      ...
      $('.social-links-wrapper .social-platforms').attr('tabindex', '0');
      ...
      var setMegaMenuWrapperHeight = function(heightValue) {
        var extraHeight = 50;
        var finalHeight = heightValue + extraHeight;
        var $el = $('.region-header-below .mega-menu-wrap');
      ...
      var calculateMegaMenuWrapperHeight = function($el) {
        if ($(window).width() > 1023) {
          var fieldHeight;
          fieldHeight = $el.closest('.menu-dropdown-0').find('.field--name-field-menu').outerHeight(true);
          var nextDropdownHeight = $el.next('.menu-dropdown').outerHeight(true) || 0;
          var level1Height = $el.closest('.menu-dropdown-0 .menu-level-1').outerHeight(true);
          var dropdown0Height = 0;
          dropdown0Height = fieldHeight + level1Height;
          var currentDropdown = $el.closest('.menu-dropdown').outerHeight(true);
          var currentMenu = $el.closest('.menu-dropdown');
          var currentHeight = currentDropdown + fieldHeight;
          var addNextHeight = fieldHeight + nextDropdownHeight;
          dropdown0Height = dropdown0Height + fieldHeight;

          if ($el.parent('li').closest('ul').eq(0).hasClass('menu-level-0')) {
            $el.parent('li').find('.menu-dropdown-0 .mega-menu-wrap').css('height', 'auto');
          } else if (currentMenu.hasClass('menu-dropdown-0')) {
            var gotoLink = $el.closest('.menu-dropdown-0').find('.field--name-field-goto-link a').outerHeight(true);
            var textLong = $el.closest('.menu-dropdown-0').find('.field--type-text-long').outerHeight(true);
            var menuLevel1 = $el.closest('.menu-dropdown-0').find('.menu-level-1').outerHeight(true);
            currentDropdown = gotoLink + menuLevel1;
            var baseHeight = gotoLink + textLong + menuLevel1;
            if (nextDropdownHeight > currentDropdown) {
              setMegaMenuWrapperHeight(addNextHeight);
            } else {
              setMegaMenuWrapperHeight(baseHeight);
            }
          } else {
            if (nextDropdownHeight > dropdown0Height && nextDropdownHeight > currentDropdown) {
              setMegaMenuWrapperHeight(addNextHeight);
            } else if (nextDropdownHeight > dropdown0Height && nextDropdownHeight < currentDropdown) {
              setMegaMenuWrapperHeight(currentHeight);
            } else if (nextDropdownHeight < dropdown0Height && nextDropdownHeight < currentDropdown) {
              if (dropdown0Height > currentDropdown) {
                setMegaMenuWrapperHeight(dropdown0Height);
              } else {
                setMegaMenuWrapperHeight(currentHeight);
              }
            } else if (nextDropdownHeight < dropdown0Height && nextDropdownHeight > currentDropdown) {
              ...
              setMegaMenuWrapperHeight(currentHeight);
            } else if (nextDropdownHeight > currentDropdown && nextDropdownHeight < dropdown0Height) {
              if (dropdown0Height > currentDropdown) {
                setMegaMenuWrapperHeight(dropdown0Height);
              } else {
                setMegaMenuWrapperHeight(currentDropdown);
              }
            } else {
              setMegaMenuWrapperHeight(dropdown0Height);
            }
      ...
      $('.region-header-below .menu-item--expanded > a').once().on('click', function(e) {
        ...
        if($(this).parent('li').hasClass('selected')) {
          $(this).parent('li').removeClass('nav-current selected');
          $(this).parent('li').find('.selected').removeClass('nav-current selected');
          $(this).closest('ul').parents('.selected').eq(0).addClass('nav-current');
          $('.nav-overlay').addClass('hide');
        }
        else {
          $(this).closest('ul').find('li').removeClass('nav-current selected');
          $(this).parent('li').addClass('nav-current selected');
          $(this).closest('ul').closest('.selected').removeClass('nav-current');
      ...
      // Show all active menu tabs expanded.
      $('.menu-level-0 > li.menu-item--active-trail > a.is-active').once('nestle_corp').on("click", function() {
        $('.menu-level-1 li.menu-item--active-trail').addClass('selected');
        var lastExpandedActiveTrailItemLink = $('.region-header-below li.menu-item--expanded.menu-item--active-trail.selected > a').last();
        calculateMegaMenuWrapperHeight(lastExpandedActiveTrailItemLink);
      });


      $('.hamburger-menu').once().on('click', function () {
        $('.header-below-wrapper').removeClass('search-mobile-wrap');
        if (!$('.header-below-wrapper').hasClass('menu-mobile-wrap')) {
        ...
        $('.header-below-wrapper #block-exposedformglobal-searchglobal-search-block-all-2').hide();
      ...
      $('.search-icon').once().on('click', function () {
        $('.header-below-wrapper').removeClass('menu-mobile-wrap');
        $('.nav-overlay').addClass('hide');
        if (!$('.header-below-wrapper').hasClass('search-mobile-wrap')) {
          $('.header-below-wrapper').addClass('search-mobile-wrap');
        }
        else {
          $('.header-below-wrapper').removeClass('search-mobile-wrap');
        }
        $('.header-below-wrapper > .region-header-below > nav').hide();
        $('.header-below-wrapper #block-exposedformglobal-searchglobal-search-block-all-2').show();
      });
      ...
      $('#flickrcontainer .searchInfo a').once().on('click',function() {
        var _this = $(this);
        $(document).ajaxComplete(function(event, xhr, settings) {
          var id = '#'+_this[0].id;
          $('#flickrcontainer .searchInfo a').removeClass('selected');
          $(id).addClass('selected');
        });
      });
      // recaptcha validation
      $(".webform-submission-contact-us-form").submit(function() {
      ...
      $( ".webform-submission-form input, .webform-submission-form .error" ).once().wrap( "<div class='input-error'></div>" );
      ...
      $('#block-secondarymenu .menu-item--expanded a').on('click', function() {
        $('#block-mainnavigation-2 .menu-item--expanded').removeClass('nav-current selected');
      });
      $('#block-mainnavigation-2 .menu-item--expanded a').on('click', function() {
        $('#block-secondarymenu .menu-item--expanded').removeClass('nav-current selected');
      });
      // Show hide Global menu
      $('#block-globallinksmenu .language-select').once().on('click', function() {
        $('#block-globalmenu').slideToggle();
        $('img.select-arrow').toggleClass('rotate-arrow');
        $('.menu li').removeClass('nav-current selected');
        $('.header-below-wrapper').removeClass('menu-mobile-wrap');
        $('.region-header-below .menu-item--expanded > a').removeClass('active-menu');
        $('.nav-overlay').addClass('hide');
        $('.menu--global-menu > .global-submenu > a').focus();
      });
      $('.close-location-region').once().on('click', function() {
        $('#block-globalmenu').slideUp();
        $('img.select-arrow').toggleClass('rotate-arrow');
        $('#block-globallinksmenu .language-select').focus();
      });
  ...
  function hideAccordion() {
    $('.view-id-map_locator .ui-accordion-header').next().css('display', 'none');
    $('.view-id-map_locator .ui-accordion-header').removeClass('ui-accordion-header-active ui-state-active');
  }
  ...
  Drupal.behaviors.nestle_corp = {
    attach: function (context, settings) {
      //Accordeon height toggle behavior.
       $(".ShellAccordeonContainer .accordeon .opener").once().on("click", function() {
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.customslick = {
    attach: function (context, settings) {
      $('div[class*="teasercycle-style-ext-"] div[id*="-c-teasercycle-item-default"] .slick-next').once().on('click touchstart', function(event) {
        $('.slick-slider').slick('slickSetOption', 'slidesToScroll', 1);
      });
      $('div[class*="teasercycle-style-ext-"] div[id*="-c-teasercycle-item-default"] .slick-prev').once().on('click touchstart', function(event) {
        $('.slick-slider').slick('slickSetOption', 'slidesToScroll', 1);
      });
      $('div[class*="teasercycle-style-ext-"] div[id*="-c-teasercycle-item-default"]').on('afterChange', function(event, slick, currentSlide, nextSlide) {
        slick.options.slidesToScroll = 3;
        if ($(window).width() < 1023) {
          slick.options.slidesToScroll = 2;
        }
      });
       $('div[class*="teasercycle-style-ext-"] div[id*="-c-teasercycle-item-default"]').on('beforeChange', function(event, slick, currentSlide, nextSlide) {
        slick.options.slidesToScroll = 3;
        if ($(window).width() < 1023) {
          slick.options.slidesToScroll = 2;
        }
